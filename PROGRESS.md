...
> 마지막 갱신: 2026-07-07 (✅ **M1b 드래그 컨텍스트 모드 구현 완료** — build+test 통과, `npm run dev` 부팅 확인. **사용자 플레이테스트 게이트 대기 중(필드 최상단 유닛 한손 둥지 투입 / 존 밖 오드롭 시 무반응 / 오판매 소멸) — 통과 전 M2 착수 금지.** 상세 기획: `docs/redesign/05-design-v3.md`)
