...
> 마지막 갱신: 2026-06-16 (출시 폴리시 후보 GD1~GD7 등록 / 자동회복 config 1→2 통일. 다음: G4 풀플레이 확인 → GD1·GD2 즉시 투입 → G5 재논의)
...
**보스 페이즈 전환 피드백 부족** — 사용자 보고: 페이즈 구분이 체감 안 됨. → **GD1(보스 페이즈 전환 컷인)로 채택, P1 즉시 투입** (아래 출시 폴리시 후보 참조).
...
| # | 박자 | 항목 | 내용 | 난이도 |
|---|---|---|---|---|
| GD1 | Pop/Plan | **보스 페이즈 전환 컷인** | Phase A→B→C 전환 시 0.4s 프리즈 + 색 변화(붉→보라→검) + "PHASE 2" 배너(1.2s 페이드). `BOSS_PHASE_B/C_START_MS` 경계 + Phase C `BOSS DOWN` 컷인 코드 재사용. **전투 입력 차단 X**. 적 30+ 시 배너만(슬로우 생략). 사용자 "페이즈 체감 안 됨" 직접 해소 | 하 |
| GD2 | Plan | **스테이지 위협 브리핑** (구 B) | StageSelect 카드에 적 구성 3줄 ("벌 65% · 탱크 2:00부터 · 보스 ×1.5"). `WORLD_CONFIGS` 데이터 그대로 노출. **적 행동 서술만, 유닛 추천 금지**(정답 빌드 고정 방지). 3줄 상한. Plan 박자 보강 | 하 |
