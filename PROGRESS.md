...
> 마지막 갱신: 2026-05-22 (리팩토링 7/8 DragController 추출)
...
└── src/
    ...
    └── scenes/              # 🎨 Phaser 레이어
        ├── GameScene.ts     # 오케스트레이터 — 137줄
        ├── constants.ts     # RACE_COLORS, RACE_EMOJI, CENTER_X/Y, SELL_ZONE_X/Y
        ├── input/
        │   └── DragController.ts        # dragstart/drag/dragend + handleDrop 분기 — 138줄
        └── render/
            ├── EnemyRenderer.ts         # 적 스폰/이동/HP바/전투/플래시 — 194줄
            ...
            └── UnitRenderer.ts          # 유닛 생성/제거, 사거리원, ❤/zzz/🔒 오버레이, 더블클릭 잠금 — 143줄
```
...
- [x] 리팩토링 2/8 (NotificationRenderer)
- [x] 리팩토링 3/8 (HudRenderer)
- [x] 리팩토링 4/8 (PopupRenderer)
- [ ] **[1] 리팩토링 5~8/8** (백그라운드, 회귀 0) — 7/8 완료
