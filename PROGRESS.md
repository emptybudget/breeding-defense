...
> 마지막 갱신: 2026-05-25 (다수 작업 완료 + U1~U15 통합 우선순위 등재, 친구 1차 관찰 피드백 반영)
...
| # | 그룹 | 항목 | 박자 | 우선순위 |
|---|---|---|---|---|
| **U13** | 보스 차별화 | **보스 3단 페이즈화** (HP×15→25→50, 속도×0.8→0.7→0.55) | Pop+Plan | **🔴 1차 패치** |
| **U14** | 인게임 결정 | **티어 강화 시스템** (강화 점, 1·2티어만, 회당 한정) | Plan+Grind | **🔴 1차 패치** |
| **U15** | 인게임 결정 | **강화 점 분당 +1 자동 지급** (fail-safe) | Grind | **🔴 1차 패치** |
| U1 | UX 명확성 | 유닛 한도 풀 시 교배 차단 알림 + HUD 펄싱 | Grind | 🟡 P0 — 2차 |
| U2 | UX 명확성 | "사회성" → "한도+1" 라벨 변경 | Plan | 🟡 P0 — 2차 |
| ~~U10~~ | ~~UX 명확성~~ | ~~소환 위치 트랙 충돌 회피~~ | — | **✅ 완료** (스폰 버그 수정 — 22px 이내 20회 재시도, `GameState.ts`) |
| U7 | 페이싱 | 분당 곱 1.25 → 1.35 (Phase C 너무 쉬우면) | Grind | 🟢 P1 — 풀플레이 후 |
| U5 | 페이싱 | Phase C 보스 처치 컷인 연출 | Pop | 🟢 P1 — 풀플레이 후 |
| U3 | UX 명확성 | 드롭 위치 트랙 충돌 시 빈 공간 보정 (8방향 30px 나선) | Grind | 🟢 P1 |
| U6 | 보스 능동성 | 보스 등장 5초 예고 (빨간 오라+카운트다운) | Plan+Pop | 🟢 P1 |
| U8 | 무한모드 | OVERCLOCK_HP_GROWTH 1.05→1.08, _SPEED_GROWTH 1.03→1.05 | Loop | 🟢 P1 |
| U9 | 페이싱 | 최종보스 거대화 (64×64, 빨간 오라, "👑 GREAT BOSS") | Pop | 🟢 P1 |
