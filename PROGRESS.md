...
> 마지막 갱신: 2026-07-13 (🔧 **버그·계승 개선 3건 (Opus 4.8, 사용자 리포트)** — ①**오디오 rejection 제거**: Phaser 내장 사운드 미사용(전부 커스텀 `SoundManager`/자체 AudioContext, grep 0건)인데 Phaser가 오디오 디바이스를 잡으려다 `[PROMISE] failed to start the audio device` unhandledrejection 발생 → `main.ts` 게임 config에 `audio: { noAudio: true }` 추가로 차단. ②**합성 특성 풀링**(사용자 결정): 합성 계승이 정점 1기 특성만 남기던 것을 **재료들의 서로 다른 특성 최대 2개**로 확장(`inheritOnSynthesis`가 sorted 우선순위대로 각 60% 생존 롤 → `trait`+`trait2`, 28-schools 2유파 상한과 정합). **계보(가문) 정체성은 여전히 단일 apex 계승**(가문 체인 불변), gen은 기존대로 max. 단일 특성이면 기존 60% 계약 그대로. `applyInheritance`에 `trait2` 적용 1줄. 신규 테스트 2건(풀링·dedup). ③이번 계승 조사에서 확인: **특성은 아직 전투 무효과**(combat.ts는 trait 미참조, 효과는 v1.1 28-schools) — 오늘 풀링은 표식/미래 투자, gen이 유일한 실파워라 합성은 이미 최댓값 계승. tsc·esbuild→node 전부 PASS. **커밋 대기.**)
