...
| # | 기능 | 출처 | 박자 | 비고 |
|---|---|---|---|---|
| A | 적 40마리 이상 → 화면 테두리 붉은 맥동 | VS | Grind | 구현비용 最低, 효과 確實 |
| B | 합성 가능한 유닛 쌍 → 빛나는 테두리 강조 | 운빨존많겜 | Grind | 텍스트 알림은 과함, 테두리만 |
| C | 웨이브 예고창 (다음 BOSS 예고 1줄, 우상단) | 히랜디 | Plan | 상세 예고는 긴장감 죽임, BOSS만 |
| D | 티어별 킬 카운터 + 보너스 골드 (2티어 50킬 → +30G) | 히랜디 | Grind | 단순 HUD 1줄로 시작 |
| E | 보스 10초 이내 처치 → 보상 카드 +1장 | 운빨존많겜 | Grind+Pop | 첫 보스는 타임 보너스 없이 시작 |
...
| # | 기능 | 출처 | 박자 | 비고 |
|---|---|---|---|---|
| G | 3/4티어 완성 시 0.3초 슬로우 + 화면 플래시 ULTIMATE 연출 | VS | **Pop** | 슬로우 0.5초 이상 금지 |
| H | 킬 스트릭: 1초 내 5킬 → 보너스 골드 +10G | VS | Pop | combat.ts에 변수 1개 추가 |
| I | 소환 1회 리롤 (골드 5G, 소환 비용 50%) | 히랜디 | Plan | 너무 싸면 랜덤 묘미 소멸 |
| J | 전방/후방 배치 존 분리 (근접 앞줄 / 원거리 뒷줄) | 히랜디 | Plan | 모바일 360px에서 구역 색만 표시, 드래그 자유도 유지 |
| K | 골드 갬블 버튼 (보유 50% 걸어 2배 or 소멸) | 운빨존많겜 | Grind | 위험도 높음 — 실제 필요성 검증 후 도입 |
| L | 합성 레시피 북 버튼 (전체 레시피 트리 상시 열람) | 히랜디 | Plan | 탭 시 일시정지 필수, 풀스크린 오버레이 |

### 7월 마일스톤 이후

| # | 기능 | 출처 | 박자 | 비고 |
|---|---|---|---|---|
| M | 스테이지별 필드 기믹 (예: Stage 2 = 트랙 2줄 분기) | Legion TD | Plan+Loop | 스테이지 시스템 확장과 묶어서 |
| N | 오버클록 진입 시 BGM 전환 | VS | Loop | 사운드 에셋 도입 후 1순위 |
| O | 유닛 공격 횟수 누적 → 레벨업 (dmg +20%) | Merge Mansion | Grind | 대격변 Phase E 이후로 미룸, UnitData 구조 변경 필요 |
