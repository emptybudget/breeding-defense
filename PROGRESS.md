...
> 마지막 갱신: 2026-05-23 (대격변 완료 + 재미 증대 우선순위 확정)

## 🚨 최우선 다음 작업 — 재미 증대 1순위 3개

대격변(Phase A~F) 구현 완료. 다음 세션은 아래 순서대로 바로 진행.

| 순서 | 기능 | 난이도 | 핵심 파일 |
|---|---|---|---|
| **1** | 3/4티어 완성 시 **ULTIMATE 연출** (0.3초 슬로우 + 화면 플래시) | 중간 | `GameScene.ts`, `PopupRenderer.ts` |
| **2** | 적 40마리 이상 → **화면 테두리 붉은 맥동** | 쉬움 | `GameScene.ts` |
| **3** | 합성 가능 유닛 쌍 → **빛나는 테두리 강조** | 쉬움 | `UnitRenderer.ts`, `DragController.ts` |
