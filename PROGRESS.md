...
> 마지막 갱신: 2026-05-21 (마일스톤 섹션 신설 — 5월 테스트버전 목표 확정)
...
- [ ] 리팩토링 2~8/8 (백그라운드 진행, 회귀 0)
- [ ] DEV값 → 출시값 적용 (`VICTORY_TIME_MS` 420000=7분 / `ENEMY_SPAWN_INTERVAL_MS` 5000)
- [ ] 풀 7분 플레이 1~2회 → 미친 수치 1~2개만 즉시 조정
- [ ] 첫 화면에 1줄 안내 ("같은 종족=교배(개체↑) / 다른 종족=합성(티어↑)")
- [ ] Firebase Hosting 배포 + 공유 URL 확보
- [ ] 본인 폰 + 친구 1명 사전 테스트 → 치명 버그만 픽스
- [ ] 친구 2~5명 본격 테스트 + 피드백 수집
...
| 5/21~22 | 리팩토링 2~8 |
| 5/23 | DEV값 원복 + 밸런싱 |
| 5/24 | 한 줄 안내 + 시작 카운트다운(선택) |
| 5/25 | Firebase Hosting 배포 + 본인 폰 검증 |
| 5/26~27 | 사전 친구 1명, 치명 버그 픽스 |
| 5/28~31 | 친구 본격 테스트, 피드백 누적 |
