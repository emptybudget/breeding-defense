...
> 마지막 갱신: 2026-07-16 (🧪 **P0-3 완료 — T3-only 승리 천장 시뮬 (Opus 4.8)** — `scripts/sim-t3-ceiling.ts` 신설(sim-freeze 하네스 재사용, 실제 GameState+runCombat 7분 시계열, 60시드/셀, 보스 HP 실제 공식 교체, 보상 emergent). **게이트 PASS**: T3 상비군(T4 0기)이 W2-1~W3-5 전부 100% 승리(PhaseC 보스 14~28s 처치, 개체수 peak≤13/40) / T2-only 67→37% 붕괴(판별력 실증) / T4 기준선 100%. FLOOR(무보상)에선 T3 붕괴 → **T3 Phase C 생존은 보스 보상 경제 의존**(의도 설계). ∴ **34번 A안/C안 보류 유지, D 종결.** tsc·esbuild→node PASS, 커밋 대기. 34번 §4·§5 갱신. ⚠️ best-case 배치 천장 테스트 — 실전 마진은 M5 W2-5 풀플레이 게이트에서 체감 확인.)
> 2026-07-16 (🧭 **출시 마일스톤 스파인 M6~M10b 확정 (Fable 5, 사용자 결정 2건, 코드 무변경)** — M5 이후 부유하던 4계열(21-G·29-S·30-V·23-L)을 M1a~M5 문법의 단일 체인으로 재배열(`docs/redesign/35-milestone-roadmap.md` 신설 — **M5 이후 순서의 유일 원천**, 21번 §투입 순서표는 폐기): **M6 「가문 시험」**(V2 무한모드 스케일링·점수식+G2 적 3종+P2-3 일격 재측정) → **M7 「소리와 언어」**(G3 BGM 3트랙+S-F ko.ts 소급+이모지 ≤20 마감) → **M8 「연결」**(Firebase 1회 구축=G5 클라우드+G1 주간 시드 리더보드+V5 일일 런) → **M9 「유파」**(V4=L1 승격+34번 A/C 재개 판단+도전과제 카탈로그 기획) → **M10a 「창문」**(S-B 해상도 스파이크 선행→S-A 판형 C안) → **M10b 「이륙」**(BM 분기·Steamworks·데모·스토어). 사용자 확정: ①출시 전개=**볼륨 완성 후 출시**(30번 기조, 모바일 소프트런칭 선행 기각) ②체인 공식 채택. 편입 전수 매핑 잔여 0(GD7만 출시 후 v1.1). v1.1 캘린더 조정: L1 선반영→분기1 슬롯=L6 시즌틀. M5 잔여 작업(§📌)은 불변 — M5 마감 후 이 체인 진입.)
> 2026-07-13 (🧬 **T3 운용성 디벨롭 — B+D 확정, 34번 신설 (Fable 5, 사용자 결정, 코드 무변경)** — 32번(영혼 촉매)은 "T4 조기 강림"만 해결, "T3를 잘 쓰는 법 모르겠음"은 별개로 진단(`docs/redesign/34-t3-usability.md`): **T3 = 시스템 진공지대**(교배 불가·강화 트랙 없음·6종 중 3종 기믹 0 → 내릴 결정이 없음). 확정 처방 ①**B(역할 표면화, 메커닉·상수 무추가)**: 역할 라벨 6종(`ko.ts` `unit.role.*`, 34번 §2 canonical 표) 유닛시트·레시피북 노출 + Gen 승계 힌트 1줄 + **Griffin=혈통 일격 엔진 공식화**(200ms 공속×STRIKE_PERIOD → Gen3 초당 일격 1.67 vs Dino 0.22, W2-5/W3-5 ⭐⭐⭐ 최적해) → **P1 가문 UI 세션에 편입**. ②**D(T3-only 승리 시뮬 = P0-3 신설)**: T4 0기로 W2/W3 Phase C 승리 가능성 미검증(보스 HP 65/15는 Gen3 T4 기준 튜닝) — `scripts/sim-t3-ceiling.ts`로 게이트, fail 시 노브 후보 3개만 보고(자동 적용 금지). **A안(T3 영혼 강화 트랙)·C안(합성 선택권 개편)은 D 결과 볼 때까지 보류.**)
> 2026-07-13 (⚖️ **T4 영혼 촉매 + ⭐⭐⭐ 배정표 확정 (Opus 4.8, 사용자 결정)** — ①**T3 메인 운용화**(`docs/redesign/32`): T4 Astral_God가 T3 대비 유효 DPS 10~13배(공10·0.3초·8타겟)라 T3를 obsolete → **T4 합성(T3×3)에 영혼 `TIER4_SYNTHESIS_SOUL_COST=3` 추가 요구**(A안). 혈통 승계는 그대로(神은 여전히 당신 혈통에서 태어남 — 영혼은 승격 촉매지 T4 구매 아님, 순수 상점구매는 기각). 비용 3 근거 = 영혼 극희소(보스 보상 2장1택 경쟁, 판당 2~3개)라 사용자 초안 "10"은 T4 봉쇄 → "잘 굴린 한 판에 1기". `GameState.synthesize()` T4 분기에 영혼 게이트+차감, 테스트 2건(부족 차단·충분 탄생/차감), tsc·esbuild→node PASS. **후속(미착수): 플레이에서 T4 1기가 여전히 T3 obsolete면 B안(T4 필드 상한 1) 검토 / 12-F2 재검증.** ②**⭐⭐⭐ 15-스테이지 배정표 확정**(`docs/redesign/33`, 사용자 "ok"): 조건 풀 7종 점증 배치(변이 1→3→5·이계열 2→4·예산 3→2·일격 20→40·3계열 정점). 신규 카운터 3개(crossBreed·sell·strikeTotal) 명세. **이건 P1 판정 코어의 설계 확정분 — 구현(judge 순수함수+카운터+결과카드)은 다음.** )
> 2026-07-13 (🔧 **버그·계승 개선 3건 (Opus 4.8, 사용자 리포트)** — ①**오디오 rejection 제거**: Phaser 내장 사운드 미사용(전부 커스텀 `SoundManager`/자체 AudioContext, grep 0건)인데 Phaser가 오디오 디바이스를 잡으려다 `[PROMISE] failed to start the audio device` unhandledrejection 발생 → `main.ts` 게임 config에 `audio: { noAudio: true }` 추가로 차단. ②**합성 특성 풀링**(사용자 결정): 합성 계승이 정점 1기 특성만 남기던 것을 **재료들의 서로 다른 특성 최대 2개**로 확장(`inheritOnSynthesis`가 sorted 우선순위대로 각 60% 생존 롤 → `trait`+`trait2`, 28-schools 2유파 상한과 정합). **계보(가문) 정체성은 여전히 단일 apex 계승**(가문 체인 불변), gen은 기존대로 max. 단일 특성이면 기존 60% 계약 그대로. `applyInheritance`에 `trait2` 적용 1줄. 신규 테스트 2건(풀링·dedup). ③이번 계승 조사에서 확인: **특성은 아직 전투 무효과**(combat.ts는 trait 미참조, 효과는 v1.1 28-schools) — 오늘 풀링은 표식/미래 투자, gen이 유일한 실파워라 합성은 이미 최댓값 계승. tsc·esbuild→node 전부 PASS. **커밋 대기.**)
> 2026-07-13 (📌 **M5 잔여 계획 점검 — 전 항목 방향 확정** (Fable 5, 코드 무변경). 결정 5건 + P0~P2 우선순위표를 "🚨 다음 작업" 최상단에 신설(§📌 M5 잔여 작업 우선순위): ①가문 시드·2차 트리 → **v1.1(L2) 공식 이월**(영구 💀 화폐와 한 묶음) ②**i18n 키 구조 확정**(`src/game/text/ko.ts` 순수 딕셔너리, `도메인.화면.항목` flat key, M5 UI 신규 문자열부터 필수) ③`breeding.ts:72` do/while 상한 = P0 선행 픽스 ④결과 카드(원인·별점·계보 체인)를 M5 가문 UI 세션에 편입(별점=30번 확정 기준) ⑤vitest는 Node≥20.19 환경 재시도 후 실패 시 vite5 호환 라인(^1.x) 다운그레이드 확정(추가 상의 불요). M5 표 갱신(계보 등록 ✅·스키마 v3 반영·시드/트리 취소선). 열린 사용자 게이트 3건(31번·M4·프리징 소킹)을 한 플레이 세션에 묶어 명시.)
> 2026-07-12 (🏛️ **M5 가문 계보 등록 — 데이터층 완료** (Opus 4.8). 판 종료(승/패) 시 apex 혈통 체인을 `MetaProgress.families`에 영구 등록. 신규 `src/game/family.ts`(순수 `buildFamilyRecord`: apex=최대 childGen·동률 최신 → 그 혈통만 gen 오름차순, 8캡=시조+상위7). 플러밍: 교배 `pedigree.push`에 `lineageId/name/epithet` 추가 + **융합도 `applyInheritance`에서 pedigree 로깅**(시조→교배→융합 체인 완성). `types.ts` `PedigreeNode` +3 옵셔널, 신규 `FamilyChainNode`/`FamilyRecord`. `MetaProgress`: `families[]`+`registerFamily`(FIFO `FAMILY_SLOT_MAX=20`)+**스키마 v2→v3**(`SAVE_SCHEMA_VERSION=3`, `families: p.families ?? []`). `GameScene`가 victory/gameover 1회 `registerFamilyOnce()`. 상수 `CHAIN_NODES_MAX=8`. 테스트 `family.test.ts`+`MetaProgress.test.ts`(v3 3건), tsc 통과, esbuild→node 실증 27/27(빌더·마이그레이션·통합). **미완(가문 UI, 다음): 혈통서 탭·결과 카드 계보 체인·3탭 스테이지맵.** 가문 시드/2차 트리는 영구 💀 화폐 미확정으로 보류(사용자 결정 선행).)
> 2026-07-12 (🎨 **명가 패치 아트 1차 반입 — NovelAI 21장 전량 생성·후처리·배치 완료** (Opus 4.8). 사용자가 `bd.zip`(A1+A2+A3 21장)을 git으로 전달 → jimp 파이프라인(`scratchpad/process.js`, 프로젝트 밖 임시)으로 **흰배경 코너 BFS 플러드필 누끼→정사각 크롭(여백 6%)→256² 리사이즈**. fx 2종은 **검정배경 유지**(가산 ADD용, 누끼 X), 컷인/키비주얼은 **코너만 누끼+원본 사이즈 유지**. 배치: `characters/`(T3 6·T4 idle+attack·알 3), `enemies/`(6), `fx/`(2), `ui/`(cutin·keyvisual). **코드 등록: `CHARACTER_ASSETS`에 T3 6종+T4 1종 추가 → GameScene preload가 자동 로드하여 T3/T4 이모지→스프라이트 즉시 교체** (`npx tsc --noEmit` 통과). `bd.zip`은 번들 방지 위해 public에서 제거(원본은 git 히스토리 a6fcdd1에 보존). 아트 품질: **T4는 chibi를 깨고 무하식 신(가챠급)으로 재설계**(사용자가 Vibe chibi anchor OFF 후 성공), greatboss 위협 강화. **✅ render 배선 6종 전부 완료(같은 세션)**: ①적 6종 이모지→enemy 스프라이트(`EnemyRenderer` Enemy 타입 Text→Image, 표시 px 상수화 40~78, setTint 유지) ②알 부화 Graphics→`egg_<family>` 스프라이트(`EGG_TEXTURE_OF_FAMILY`+`eggFamily()` lineage→family 폴백, 게이지 아크 r28) ③fx: 혈통일격→`fx_bloodline_strike` 발사체(ADD, scale 0.6→1 이동 120ms)·전설/희귀 부화→`fx_hatch_burst`(ADD, 전설 순금 1.4배/희귀 은tint 0.9배/일반 미사용) ④W1-5 컷인→`cutin_astral_god` 이미지(폭 340, 텍스트 폴백 유지) ⑤`TitleScene` 키비주얼 커버 배경 ⑥Astral_God divine 공격 시 `_attack` 프레임 0.15s 교차. 전 배선 **텍스처 부재 시 기존 스텁 폴백** 유지, `npx tsc --noEmit` 통과, dev 5173 자산 200 확인. ⚠️ **미완(별건): M5 "이모지 grep ≤20"** — 적 렌더 이모지는 0이나 repo 전역 ~200(대부분 정당 UI 💰💎 + `RACE_EMOJI`/`RACE_COLORS` 폴백). 폴백 제거는 별도 정리. 프롬프트 전량+Vibe-OFF 경고는 Artifact `art-prompts`.)
> 2026-07-10 (🏛️ **M5 착수 — 아트 비의존 1차분: F1 밸런스 + 특성 W2-1 게이팅** (Opus 4.8). ①F1: `BOSS_HP_PHASE_C_SCALAR` 50/15→**65/15**(12-F1, Gen3 T4 처치 2.6s→3.3s 복원). ②특성 W2-1 해금: `StageFeatures`에 `traits` 플래그 신설(W1-1~1-5 전부 false, W2+ ALL_FEATURES=true), `completeBreeding`이 `!features.traits`면 자식 `trait/trait2` 미부여(칭호 epithet은 M3 변이 표기라 게이팅 제외). 신규 테스트 `traitGating.test.ts`(esbuild→node 실증 통과). tsc·dev200 통과. **⚠️ 잠재버그 발견(미수정, M3 선존재): `breeding.ts:72` 전설 2번째 특성 롤 `do/while`에 반복 상한이 없어 rng가 degenerate(상수)면 무한루프 — 실제 Math.random에선 사실상 무발생이나 테스트에서 rng=0 고정 시 hang 확인. 다음에 iteration cap 추가 권장.** **가문 계보 등록(스키마 v3+마이그레이션+체인 빌더)은 이번에 제외 — 사용자가 곧 혈통서/스테이지맵 아트 제작 예정이라 UI와 함께 다음 세션에 묶어서 처리(half-built 마이그레이션 push 회피). 설계 조사분은 아래 M5 착수 메모 참조.** 커밋+push 완료.)
...
- F1(트랙 3층 리비주얼)·F2(판정 반폭 22→18)·F3(드롭 관용화)는 직전 세션(커밋 a44ae55)에서 이미 완료 — 이번 세션은 **F4 하나만** 처리: `DragController.ts:326`의 고정 문구 `'⚠️ 같은 카테고리 유닛만 교배 가능'`(이계열 교배가 정식 루프라 오정보였음)을 `sendToNest`(같은 파일 346-350행)와 동일한 예산소진/잠금 분기로 교체. 로직 변경 없음, 문구만 통일.
- F2 게이트가 요구한 "경계 17.9/18.1 테스트 1건"이 어느 기존 파일에도 없어 `tests/game/track.test.ts` 신규 작성. `trackWaypoints`가 판마다 랜덤(5종 레이아웃+지터)이라 결정론적 테스트가 안 돼서, `Math.random`을 0.5로 모킹해 지터 0인 고정 육각형 레이아웃을 강제한 뒤 세그먼트 0의 중점에서 수직으로 17.9px/18.1px 떨어진 점을 계산해 `isOnTrack` 참/거짓을 검증(하드코딩 좌표 대신 실제 `trackWaypoints`에서 기하 계산 — 레이아웃 상수가 바뀌어도 테스트가 안 깨짐).
- **`npm test`가 여전히 이 환경에서 기동 불가**(M2~M4와 동일한 Node v20.18.1 + `@rolldown/binding` 네이티브 바인딩 이슈, 이번 세션에 재확인 — `Cannot find module '@rolldown/binding-linux-x64-gnu'`). esbuild로 신규 테스트 로직만 번들 → 순수 Node 실행으로 PASS 확인(`GameState` 자체가 Phaser 무의존이라 이 방식이 계속 유효). **Firebase Studio 등 다른 Node 버전 환경에서 `npm test` 재확인 권장**(반복되는 이슈 — 다음에 실제로 vitest가 도는 환경을 만나면 vitest 버전을 vite 5 호환 라인으로 낮추는 걸 사용자와 상의할 가치 있음).
- 남은 것은 §2 게이트의 사용자 플레이 확인뿐(코드 변경 없는 항목) — 통과 시 31번 완료 처리하고 M5(Opus 4.8) 착수.
...
### 📌 M5 잔여 작업 우선순위 (2026-07-13 확정, Fable 5) — 여기부터 순서대로

**결정 5건 (즉시 유효):**

1. **가문 시드(💀3)·2차 트리 → v1.1(L2 가문 전당)로 공식 이월.** 둘 다 영구 💀 이월 화폐를 전제하는데 획득 경로 미확정 + BM(11번)과 얽혀 헌법 심사 필요. M5는 "계보 UI+리스킨+밸런스"로 닫아 출시 가능 단위 유지. L2 착수 시 영구 💀 획득 경로 설계를 선행 과제로 함께.
2. **i18n 키 구조 확정** (29번 "M5 전 선결정"의 실행 — 문서 부재 상태였음): ①순수 TS 딕셔너리 `src/game/text/ko.ts` (라이브러리 도입 없음, 데이터/렌더 분리 준수, `t('key')` 헬퍼 1개) ②키 = `도메인.화면.항목` flat string (예: `ui.pedigree.tab`, `result.defeat.tipSummon`) ③장문 로어(`lore.ts`의 birthCry/dex/CHRONICLE)와 혈통 네이밍(naming.ts)은 기존 구조 유지 — 이미 키화된 데이터라 이중화 금지 ④적용 = **M5 가문 UI 신규 문자열부터 필수**(이후 신규 하드코딩 한글 금지), 기존 문자열 소급은 Steam 빌드 준비 때 일괄.
3. **`breeding.ts:72` do/while 상한 (M3 선존재 잠재버그)**: 다음 코드 세션 첫 작업으로 선행 편입. 스펙 — 반복 상한 12회, 초과 시 `(ALL_TRAITS.indexOf(inheritedTrait)+1) % ALL_TRAITS.length`로 결정론 폴백(중복 불가 보장 유지). rng 상수 고정(degenerate) 테스트 1건 동반.
4. **결과 카드(GameOver/Victory: 원인·별점·계보 체인) = M5 가문 UI 세션에 편입** (M4 이연분 unblock — 별점 기준이 30번에서 확정됨: ⭐승리/⭐⭐Gen3 정점/⭐⭐⭐혈통 도전). 계보 체인은 `buildFamilyRecord`/`MetaProgress.families` 그대로 재사용.
5. **vitest 환경 종결 규칙**: 다음 Node≥20.19 환경(Firebase Studio 등)에서 `npm test` 재시도 → 여전히 실패 시 vitest를 vite5 호환 라인(^1.x)으로 다운그레이드 **확정(추가 상의 불요)**. 그때까지 esbuild→node 실증 우회 유지. 세션마다 이 이슈 재발견 금지 — 이 항목만 참조.

**작업 순서:**

| 순위 | 작업 | 담당 | 비고 |
|---|---|---|---|
| P0-1 | ✅ **완료 (2026-07-13, Opus 4.8)** — `breeding.ts:70` 전설 2슬롯 do/while 상한 12회 + 초과 시 `(indexOf+1)%12` 결정론 폴백. 종료-검증 테스트 1건(`scripted([0])`, degenerate rng 실증 1ms 종료). tsc·esbuild→node PASS | — | 커밋 대기 |
| P0-2 | ✅ **완료 (2026-07-13, Opus 4.8)** — `src/game/text/ko.ts` i18n 골격(`KO` flat dict `as const` + `t(key, params?)` `{param}` 치환, 컴파일타임 키 안전). 30번 별점 확정문구 `result.*` 시드. 테스트 `tests/game/text.test.ts` 4건. tsc·esbuild→node PASS | — | 커밋 대기. P1 신규 문자열 전부 이걸 통해 |
| P0-3 | ✅ **완료 (2026-07-16, Opus 4.8)** — `scripts/sim-t3-ceiling.ts` 신설(sim-freeze 하네스 재사용: 실제 GameState+runCombat, tick→spawn→move→combat 7분 시계열, 60시드/셀). 보스 HP=EnemyRenderer.spawnBoss 실제 공식으로 교체(옛 sim-freeze hp:75 무효), 보스 보상은 처치 시 발생(emergent, 카드 순환)으로 실제 경제 모사. **결과: ① T3 상비군(T4 0기) W2-1~W3-5 전부 승률 100%**(개체수 peak 6~13/40, PhaseC 보스를 랩 이탈 전 14~28s에 처치) / ② T2-only 대조군 67→37% 붕괴(W3-5 개체수 41 오버플로 — 시뮬 판별력 실증) / ③ T4 기준선 100%. **FLOOR(무보상) 발견: T3의 Phase C 생존은 보스 보상 경제에 의존**(W3-5 무보상 0%). tsc·esbuild→node PASS. **게이트 PASS → A안/C안 보류 유지, 34번 D 종결.** | Opus 4.8 | 커밋 대기 |
| P1 | ⏸ **사용자 지시로 보류 (2026-07-13)** — **M5 가문 UI 헤드라인 세션**: 혈통서 탭(RecipePopup 2탭) + 결과 카드(원인·별점·계보 체인, §결정4) + 3탭 스테이지맵. **재개 시 확정된 진행 방식 = "판정 코어부터"**: ⭐⭐⭐ 15-스테이지 배정표(30번 조건 풀 기반)를 AI가 초안 → 사용자 확인 → 판정 순수함수 + GameState 카운터 추가 + 결과 카드 렌더. ⚠️ 착수 전 확인분: ①⭐⭐⭐ 판정 카운터가 GameState에 **현재 0개**(crossBreed·mutationUnit·sell·strike 누적 등 신규 추적 필요) ②⭐/⭐⭐(승리·정점 Gen3+)는 즉시 판정 가능. **+ 34번 B(T3 역할 라벨 6종·Gen 승계 힌트·레시피북 병기) 이 세션에 편입** | Opus 4.8 | 데이터층 완비(`families` v3). 스테이지맵/도감·가문 문장 아트 필요분은 착수 전 사용자에게 요청(07번 AM 절차) |
| P2-1 | 이모지 폴백 정리(`RACE_EMOJI`/`RACE_COLORS` 등) → grep ≤20 | 하위 모델 | 스프라이트 전환 완료 전제, 정당 UI 이모지(💰💎)는 잔존 허용 |
| P2-2 | NovelAI 잔여 ~12장(도감 카드·스테이지맵 등) 반입·후처리·등록 | 사용자+하위 모델 | 파이프라인: 12526f9 선례 + Artifact `art-prompts` |
| P2-3 | 혈통 일격 +50% 긴장감 재측정(주기 파라미터 1개) + 12-F2 예비 조정안 재검 | 하위 모델 | 상수 1개씩 원칙(19번 헌법) |
| P2-4 | 프리징 계측 제거: `main.ts` `#fatal-error` 오버레이·워치독 + `scripts/sim-freeze.ts` | 하위 모델 | 게이트: 아래 사용자 게이트 ③ 통과 후. 스토어 스샷 촬영 전 필수 |
| P2-5 | 스토어 스샷 5장(09번 시나리오) + M5 완료 판정 | 사용자 | 이모지 grep ≤20·저장 왕복 테스트 포함(M5 표) |

**열린 사용자 플레이 게이트 3건 — 다음 플레이 세션 한 번에 처리 후 이 줄에 통과 표기:**
① 31번 §2-3,4(필드 재배치 10회 연속 원위치 0회 + 트랙 경계 식별 + 새 판 5회 레이아웃 5종) ② M4 게이트(설명 없이 둥지 발견 + 스샷에서 Gen 단계 지목) ③ 프리징 소킹(iPhone 실기기 연속 10판 무재현 → P2-4 해금)
...
> 🛠 트랙·드래그 수정 스펙: `docs/redesign/31-track-drag-fix.md` — **구현 최우선(사용자 지시)**. 원위치 원인 5개(판정>비주얼 4px, 고정 존 사각지대, 말없는 폴백 3경로) + F1~F4 수정 스펙(하위 모델 킥오프용, file:line 명시).
> 🧭 출시 마일스톤 스파인: `docs/redesign/35-milestone-roadmap.md` — **M5 이후 순서의 유일 원천** (M6~M10b, G/S/V/L 전수 편입, 볼륨 완성 후 출시 확정, 2026-07-16).
> 📦 Steam 볼륨 설계: `docs/redesign/30-steam-volume.md` — Steam 1.0 경계(월드4·침공 제외), **별점 45개 기준 확정(⭐승리/⭐⭐Gen3 정점/⭐⭐⭐혈통 도전)** = M4 이연 미결 해소, 무한 모드 「가문 시험」 정식화, 실질 6h 검산, 신규 작업 V1~V5.
> 🚂 Steam 품질 기준선: `docs/redesign/29-steam-quality.md` — 목표 재정의(Steam 즉시 출시급) 원천. 최저선 8축·판형 C안(세로 코어+사이드 패널)·BM 분기·S 시리즈 갭 감사 + 후속 디벨롭 회차 큐.
...
| 🥉 M5 | 🔄 **진행 중** — **가문 계보 + 밸런스 + 잔여 리스킨** — 판 종료 시 계보 체인 통째 가문 등록(노드 ≤8, 슬롯 상한) ✅(2026-07-12 데이터층·스키마 v3), ~~가문 시드(💀3), 2차 트리 2종~~(→**v1.1 L2 이월 확정 2026-07-13**, §📌 결정1). 타이틀 키비주얼 ✅+3탭 스테이지 맵+도감/혈통서 2탭+**결과 카드(원인·별점·계보, §📌 결정4 편입)**. NovelAI 잔여(~12장)+이모지 ≤20. 특성 해금 W2-1 ✅(2026-07-10). 혈통 일격 +50% 긴장감 재측정(주기 파라미터 1개 튜닝). **잔여 작업 순서 = §📌 M5 잔여 작업 우선순위 표** | 대 | `MetaProgress.ts`(가문, 스키마 v3 ✅), `TitleScene.ts`, `StageSelectScene.ts`, `popups/RecipePopup.ts`(혈통서 탭), `scenes/constants.ts`(CHARACTER_ASSETS) | `FamilyRecord`, ~~`FAMILY_SEED_COST=3`~~(v1.1), `FAMILY_SLOT_MAX=20` ✅, `CHAIN_NODES_MAX=8` ✅ | build / 저장 왕복 테스트(스키마 v3 마이그레이션 포함 ✅) / 이모지 grep ≤20 / 스토어 스샷 5장 촬영 가능 |

#### 마일스톤 M6~M10b — 출시 스파인 (2026-07-16 확정, 상세·완료 판정 = `35-milestone-roadmap.md`)

> 대전제(사용자 확정): **볼륨 완성 후 출시** — M6~M9는 플랫폼 중립 콘텐츠, 패키징은 M10에서만. 어느 M에서 멈춰도 출시 가능 단위. 외부 SDK 도입은 M10 전 동결(29-§4).

| # | 마일스톤 | 핵심 | 원천 | 담당 |
|---|---|---|---|---|
| M6 | 「가문 시험」 | V2 무한모드 정식화(스케일링+점수식, 시뮬 게이트) + G2 적 3종(실드·분열·힐러) + P2-3 일격 재측정 | 30-§3, 21-G2 | Opus 4.8 → Sonnet 5 |
| M7 | 「소리와 언어」 | G3 BGM 3트랙 통합(생성=사용자) + S-F 기존 문자열 ko.ts 소급 + 이모지 grep ≤20 마감 | 21-G3, 29-S-F | Sonnet 5 |
| M8 | 「연결」 | Firebase 익명 인증 1회 구축 → G5 클라우드 세이브 + G1 주간 시드·리더보드 + V5 일일 런(보상은 M10b BM에서) | 21-G1·G5, 30-§4 | Opus 4.8 |
| M9 | 「유파」 | V4=L1 승격(28번 6유파 세트 판정+인장) + 34번 A/C 재개 판단(P0-3 게이트) + 도전과제 카탈로그 기획 | 28, 23-L1 | 기획 상위, 구현 Sonnet 5 |
| M10a | 「창문」 | S-B 해상도 스파이크 **선행 필수** → S-A 판형 C안(좌 계보 뷰/우 타임라인) + Deck 검증. 모바일 코어 1px 무손상 | 29-§2·회차3·5 | Opus 4.8 |
| M10b | 「이륙」 | BM 플랫폼 분기(Steam 프리미엄/모바일 F2P+Capacitor) + Steamworks(도전과제=M9 카탈로그) + 데모(W1 경계) + 스토어 패키지 | 11, 29-S-D·E·H, 09 | Opus 4.8 + 사용자 |

- 이후 = v1.1+ 라이브옵스, 23번 캘린더 인계(L1 선반영 → 분기1 슬롯=L6 시즌틀로 대체).
- 사용자 병렬 트랙(미리 가능): 적 3종 스프라이트(M6 전)·BGM 생성(M7 전)·EN 번역(M10b 전) — 35번 §4.
